...
This page looks long, but it's mostly sections with one paragraph each. Just follow the instructions step-by-step and you'll be set.

<aside class="aside-warning">

In the interest of brevity, this document doesn't touch upon best practices for shell scripting.
...
The shell we use on CAEN is called @bash@, but it's not the only shell out there. There are many others, like @zsh@, @csh@, @ksh@... The shell script we're writing should work in any of these. (One exception is the @fish@ shell, but it's not installed on CAEN by default, anyways.)
...
You'll notice that this is the opposite of what we usually expect from boolean logic. Normally, a value of zero means @false@, but here zero means success. It's more convenient to use zero to mean success and non-zero to mean failure because there are multiple reasons a program could fail but usually only one way that it could succeed.
...
Now if we run @make test@, we'll get output either like this:
